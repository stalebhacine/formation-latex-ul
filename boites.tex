...
famille \cmd{\ieme} de \pkg{babel} (section~1.1 de la %
...
\end{lstlisting}
  ...
  deux «caractères» sur une seule ligne de «texte». La \Ie{minipage}
  ...
  pour créer une seconde ligne (vide) dans la \Ie{minipage} externe.
  \begin{sol}
    L'idée consiste à créer une seconde ligne dans la \Ie{minipage}
...
\begin{exercice}
  ...
  la police de caractères est \cs{footnotesize}.)
  \begin{center}
  ...
  \begin{sol}
    ...
    boîtes dans une \Ie{minipage} dans laquelle les deux boîtes seront
    alignées tel que désiré, puis à aligner la \Ie{minipage} avec la
    troisième boîte. Cependant, il faut insérer une seconde ligne
    invisible dans la \Ie{minipage} afin de pouvoir l'aligner par le
