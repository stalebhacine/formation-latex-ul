...
\end{lstlisting}
  ...
  \end{minipage}
\end{demo}
Or, utiliser à répétition la commande \cmdprint{text} pour entrer des
noms de fonction se révèlerait peu pratique à l'usage.
...
La commande \cmd{\mathcal} permet de transfomer un ou plusieurs
...
fournis par {\LaTeX} fréquemment utilisés en mathématiques.
...
\begin{exercice}
  ...
  \item À l'aide de la fonction Rechercher et remplacer de l'éditeur
    de texte, remplacer toutes les occurences du symbole $\theta$ par
    $\lambda$.
  \end{enumerate}
\end{exercice}
