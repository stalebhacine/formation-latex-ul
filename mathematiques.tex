...
\item L'environnement \Ie{split} est utile pour apposer un seul numéro
...
\item Les renvois vers des équations numérotées fonctionnent, comme
  ...
  \pkg{amsmath} fournit également la commande \cmd{\eqref} qui place
...
\end{lstlisting}
    ...
    \end{texample}
  \end{demo}
\item À l'instar de \Ie{equation}, tous les environnements du
  \autoref{tab:math:displays} sauf \Ie{split} ont une version étoilée
  (\cmdprint{align*}, \cmdprint{gather*}, etc.) qui produit des
  équations non numérotées.
\end{itemize}
Consulter la %
\doc[documentation]{amsldoc}{http://texdoc.net/pkg/amsmath} %
(chapitre 3) du paquetage \pkg{amsmath} pour les détails sur
