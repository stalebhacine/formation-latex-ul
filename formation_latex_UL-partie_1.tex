...
\begin{lstlisting}
\documentclass[`\alt<2>{\textbf{french}}{french}']{memoir}
  \usepackage{`\alt<2>{\textbf{babel}}{babel}'}
  \usepackage[autolanguage]{`\alt<5>{\textbf{numprint}}{numprint}'}
  \usepackage[utf8]{`\alt<3>{\textbf{inputenc}}{inputenc}'}
  \usepackage[T1]{`\alt<3>{\textbf{fontenc}}{fontenc}'}
  \usepackage{`\alt<4>{\textbf{icomma}}{icomma}'}
\end{lstlisting}
  \begin{itemize}
  \item<2> \pkg{babel}: traduction des mots-clés prédéfinis,
    typographie française, coupure de mots, document multilingue
  \item<3> \pkg{inputenc} et \pkg{fontenc}: lettres accentuées dans le
    code source
  \item<4> \pkg{icomma}: virgule comme séparateur décimal
  \item<5> \pkg{numprint}: espace comme séparateur des milliers
  \end{itemize}
\end{frame}
