...
\item Construire une ou plusieurs bases de données contenant les
  ...
  pour tous nos documents. Par conséquent, le temps consacré à cette
  étape s'amenuise au fur et à mesure que l'on complète nos bases de
  données.
...
par le biais d'exemples. Suivent l'\autoref{ex:bibliographie:bib} des
commentaires et précisions sur la préparation des entrées
bibliographiques.

\begin{exemple}
  \label{ex:bibliographie:bib}
  On trouvera ci-dessous les entrées bibliographiquesd'un livre
...
devrait utiliser les commandes de \pkg{natbib}
\begin{lstlisting}
\citeauthor`\marg{clé}'
\citeyear`\marg{clé}'
\end{lstlisting}
...
\begin{exercice}
  ...
  \begin{enumerate}
  \item \nolink{\bibentry{Mittelbach:floats:2014}}
