...
graphiques et d'images, il convient de présenter très succinctement
quelques règles à suivre pour concevoir des tableaux clairs et faciles
à consulter.
...
\item placer les unités (\$, cm, {\textdegree}C, \dots) dans le titre
...
contiendra le tableau, l'entête du tableau, le as échéant, et le contenu
...
\cite[section \href{http://fr.wikibooks.org/wiki/LaTeX/Tableaux}{Tableaux}]{wikilivres:latex}.
...
l'environnement \Ie{picture}. La consultation du code commenté correspondant devrait
permettre de comprendre les principes de base de la création de
figures. Autrement, l'annexe~D de la documentation de la classe
\class{memoir} (\fichier{memman.pdf} dans {\TeX}~Live ou %
  \href{http://texdoc.net/pkg/memoir}{via TeXdoc Online}) %
fournit une bonne introduction à \Ie{picture}.
...
(\fichier{grfguide.pdf} dans {\TeX}~Live ou
  \href{http://texdoc.net/pkg/graphics/}{via TeXdoc Online}) %
...
\begin{lstlisting}
%% réduction à 40 % de la taille réelle
...
\end{lstlisting}
  ...
  \item %
    \begin{minipage}[t]{0.62\linewidth}
\begin{lstlisting}
%% réduction à exactement 15 mm de haut
...
consulter la documentation (\fichier{pdfpages.pdf} dans {\TeX}~Live ou %
  \href{http://texdoc.net/pkg/pdfpages/}{via TeXdoc Online}).
...
En définissant un élément comme flottant, on laisse la possibilité à
{\LaTeX} de le disposer au meilleur endroit en fonction de la taille
de l'élément, du contenu du document et de diverses règles
typographiques.
...
section 10.4 de la classe \class{memoir} (\fichier{memman.pdf} dans
{\TeX}~Live ou %
  \href{http://texdoc.net/pkg/memoir/}{via TeXdoc Online}) %
explique plus en détail la signification des valeurs ci-dessus. Le
lecteur qui voudrait vraiment \emph{tout} savoir sur la disposition des
éléments flottants pourra consulter \cite{Mittelbach:floats:2014}.

\begin{exemple}
  On reprend le tableau de l'\autoref{exemple:tableaux:tabular}, mais cette
  fois défini à l'intérieur d'un environnement \Ie{table}:
\begin{lstlisting}
\begin{table}
  \centering
  \begin{tabular}{lrrr}
...
\end{table}
\end{lstlisting}
  On voit que {\LaTeX} a automatiquement placé le tableau au haut de
  la présente page.
  \begin{table}
    \centering
    \begin{tabular}{lrrr}
...
figure~3\dots»). Cela
permet de guider le lecteur au fil de sa lecture et de construire une
liste des tableaux et des figures\footnote{%
...
apparait dans l'environnement, une légende de la forme «\textsc{Tableau}
\emph{n}: \meta{texte}» pour un tableau ou «\textsc{Figure}
\emph{n}: \meta{texte}» pour une figure. Le texte de la légende est
centré sur la page lorsqu'il fait moins d'une ligne; dans le cas contraire il est disposé
...
\begin{exemple}
  ...
  le \autoref{tab:tableaux:captions}. La figure elle-même est composée
  avec le code suivant:
\begin{lstlisting}
  \begin{figure}
    [...]
    \caption{Code source pour créer le \autoref{tab:[...]}}
    \label{fig:[...]}
  \end{figure}
\end{lstlisting}
...
Les environnements \Ie{table} et \Ie{figure} créent des éléments
...
\Ie{minipage} se revèlent alors particulièrement utiles pour disposer
les éléments de contenu dans la boîte.
...
\begin{exemple}
  ...
  de boîtes verticales créées avec l'environnement \Ie{minipage}. On
