\chapter{Blocs, tableaux et figures}
\label{chap:tableaux}
...
Dans les traitements de texte, on aura souvent recours aux tableaux
pour de tels usages. {\LaTeX} offre ici une solution plus générale.
Les tableaux servent spécifiquement à disposer de l'information sous
forme de grille. Autrement, on utilise des «blocs», ou «boîtes»
(\emph{boxes} en anglais) pour la disposition et la mise en forme de
contenu quelconque se présentant sous forme rectangulaire.
...
d'autres boîtes dont nous contrôlons les dimensions ainsi que le
contenu.

Les commandes de création de blocs de texte les plus simples sont:
\begin{lstlisting}
\mbox`\marg{texte}'
\fbox`\marg{texte}'
\end{lstlisting}
Elles produisent un bloc de la largeur précise de \meta{texte}. Avec
la commande \cmd{\fbox}, le texte est au surplus \fbox{encadré}.

Le bloc créé avec \cmd{\mbox} est une sorte d'unité de base avec
lequel il est possible de construire des structures plus complexes. En
usage courant, il sert principalement à deux choses:
\begin{enumerate}
\item réunir en un bloc du texte que l'on ne veut pas voir scindé
  entre les lignes ou entre les pages;
\item avec \verb=\mbox{}=, laisser croire à {\TeX} que du contenu
...
respectivement la largeur du bloc de texte et la position du texte
dans le bloc:
\begin{description}
\item[\normalfont\code{l}] pour du texte \framebox[3.5cm][l]{aligné à gauche};
\item[\normalfont\code{c}] pour du texte \framebox[3.5cm][c]{centré} (la valeur
  par défaut);
\item[\normalfont\code{r}] pour du texte \makebox[3.5cm][r]{aligné à droite}.
\end{description}
Les blocs ci-dessus ont été créés avec les commandes suivantes:
\begin{lstlisting}
\framebox[3.5cm][l]{aligné à gauche}
\framebox[3.5cm][c]{centré}
\makebox[3.5cm][r]{aligné à droite}
\end{lstlisting}
