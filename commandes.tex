...
marqué par la déclaration \cmdprint{\begin\marg{envir}}; le contenu en tant que
  tel; la fin, marquée par la déclaration \cmdprint{\end\marg{envir}}.
...
\begin{exemple}
  ...
  partir de l'environnement standard \Ie{quote}:
\begin{lstlisting}
...
\begin{exemple}
  ...
  \Ie{conseil} servant à mettre en forme les rubriques «Conseil du
...
\end{lstlisting}
  ...
  \item crée une première \Ie{minipage} pour disposer le symbole
    {\faThumbsOUp} tiré de la police Font~Awesome;
  \item ouvre une seconde \Ie{minipage} à côté de la première, on y
...
\begin{exercice}
  ...
  \begin{quote}
    \begin{tabular}{p{\linewidth}}
      \toprule
      \small\sffamily%
      La citation est surmontée et suivie de filets horizontaux. Le
      texte est toujours en police plus petite, mais sans empattements
      plutôt qu'en italique. \\
      \bottomrule
    \end{tabular}
  \end{quote}
  \emph{Astuce}: placer le texte de la citation dans un tableau de
  largeur \cmdprint{\linewidth}, puis utiliser les commandes
  \cmdprint{\toprule} et \cmdprint{\bottomrule} pour créer les
  filets horizontaux.
  \begin{sol}
