\documentclass[aspectratio=54,xcolor=x11names]{beamer}
  ...
  \setbeamercolor{block title alerted}{fg=comments}
...
\begin{frame}[plain]
  %% Code pour créer la page couverture conservé dans un fichier séparé
  \input{couverture}
\end{frame}
...
\usepackage{`\textit{paquetage1,paquetage2,...}'}
  ...
  \item<2-> Les incontournables:
    \begin{description}
      \small
    \item[babel*] typographie multilingue
    \item[inputenc*] composition en français (\LaTeX)
    \item[fontspec*] contrôle des polices (\XeLaTeX)
    \item[amsmath] extensions mathématiques
    \item[booktabs*] amélioration des tableaux
    \item[hyperref*] hyperliens dans PDF
    \end{description}
...
\end{lstlisting}
  \item Classe \textbf{ulthese}: résumés français et anglais traités
    comme des chapitres (non numérotés) normaux
  \end{itemize}
\end{frame}
...
\begin{frame}[plain]
  \begin{exercice}
    ...
    \item Insérer deux ou trois titres de sections et de sous-sections
      dans le document.
    \end{enumerate}
  \end{exercice}
\end{frame}
...
%%% >>>
\begin{frame}[plain=singleslide]
  \begin{exercice}
    Reprendre le fichier \fichier{exercice\_classe+paquetages.tex}
    modifié à l'exercice précédent.
    \begin{enumerate}
    \item Insérer dans le texte un renvoi au numéro d'une section.
    \item Charger le paquetage \textbf{hyperref} et comparer l'effet
      d'utiliser \texttt{{\textbackslash}ref} ou
      \texttt{{\textbackslash}autoref} pour le renvoi.
    \end{enumerate}
  \end{exercice}
\end{frame}
%%% <<<
...
\begin{frame}[fragile=singleslide]
  \frametitle{Annexes}
  \begin{itemize}
  \item Annexes sont des chapitres avec une numérotation
    alphanumérique (A, A.1, ...)
  \item Prochains chapitres identifiés comme des annexes par la
    commande
\begin{lstlisting}
\appendix
\end{lstlisting}
  \item Dans le titre, «Chapitre» changé pour «Annexe»
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile=singleslide]
  ...
  \begin{block}{Famille}
    \vspace{-12pt}
    \begin{tabbing}
      \textsf{sans empattements} \quad\= \verb=\sffamily= \quad\=
...
\begin{frame}[plain]
  \begin{exercice}
    ...
    \item Ouvrir le fichier \fichier{exercice\_complet.tex} et en
      étudier le code source avant de le compiler.
    \item Effectuer les modfications suivantes au document.
      \begin{enumerate}[a)]
...
\begin{frame}[fragile=singleslide]
  \frametitle{Extrait de la documentation de \textbf{ulthese}}
  \begin{quote}
    «Il est recommandé de segmenter tout document d'une certaine
    ampleur dans des fichiers \verb=.tex= distincts pour chaque
    partie --- habituellement un fichier par chapitre.
  \end{quote}

  \begin{quote}
    Le document complet est composé à l'aide d'un fichier maître qui
    contient le préambule {\LaTeX} et un ensemble de commandes
    \verb=\include= pour réunir les parties dans un tout.»
  \end{quote}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Structure d'un fichier maître}
\begin{lstlisting}
...
\begin{frame}[fragile]
  \frametitle{Deux manières d'insérer du contenu externe}
  \begin{itemize}
  \item Commande \verb=\include= insère le contenu d'un fichier
    ...
    \end{itemize}
  \item<2-> Commande \verb=\input= permet aussi d'insérer le contenu d'un
    fichier
    \begin{itemize}
...
\end{lstlisting}
    ...
    \item lire la documentation de ce paquetage pour connaître toutes
      ses fonctionnalités
    \end{itemize}
  \item Important d'utiliser un mode mathématique pour mettre en forme
    des mathématiques
    \begin{itemize}
    \item «language» spécial pour décrire les mathématiques et il faut
      en informer {\LaTeX}
...
\end{lstlisting}
  \item «Hors paragraphe» séparé du texte principal comme
    \begin{displaymath}
      \int_0^\infty f(x)\, dx = \sum_{i = 1}^n w_i e^{x_i} f(x_i)
    \end{displaymath}
...
\begin{frame}[plain]
  \begin{exercice}
    \begin{enumerate}
    \item Examiner le code source du fichier
      \fichier{exercice\_mathematiques.tex}.
    \item Insérer ce fichier dans le document maître
      \fichier{gabarit-doctorat.tex}. Dans ce dernier, activer les
      paquetages \textbf{amsmath} et \textbf{icomma}, puis compiler.
    \item Modifier un environnement \texttt{align*} pour
      \texttt{align} et observer le résultat.
    \end{enumerate}
  \end{exercice}
\end{frame}
%%% <<<
...
\begin{frame}[plain]
  \begin{exercice}
    Compiler de nouveau le fichier maître obtenu à l'exercice
    précédent en utilisant une police de caractères différente.
  \end{exercice}
\end{frame}
%%% <<<
