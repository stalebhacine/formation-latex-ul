...
\section{Insertion de contenu avec la commande \code{{\textbackslash}input}}
\label{sec:include:input}

La commande \verb=\input= permet d'insérer le contenu d'un autre fichier
...
\section{Insertion de parties avec la commande \code{{\textbackslash}include}}
...
Comme \verb=\input=, la commande \verb=\include= insère le contenu
...
\begin{itemize}
\item L'insertion d'un fichier avec \verb=\include= débute toujours
  une nouvelle page. On utilisera donc \verb=\include= principalement
  pour insérer des chapitres entiers plutôt que pour des portions de
  texte.
\item Un fichier inséré avec \verb=\include= peut contenir des appels
  à \verb=\input=, mais pas à \verb=\include=.
\item La syntaxe de la commande \verb=\include= est
\begin{lstlisting}
...
\item Le principal avantage de \verb=\include= par rapport à
  \verb=\input= réside dans le fait que {\LaTeX} peut préserver entre
  ...
  représentative.
\item La commande \verb=\includeonly=, que l'on utilise exclusivement
...
\item Lors de l'utilisation de la commande \verb=\includeonly=, toute
...
d'utilisation des commandes \verb=\include= et \verb=\includeonly=.
