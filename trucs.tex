...
des commandes à l'invite de commande. L'expression est
immédiatement évaluée et le résultat est affiché à
l'écran:
<<>>=
2 + 3
pi
...
\item Le fichier \code{.Rnw} est passé à la commande \code{Sweave()}
  ...
  dans des environnements \Ie{Sinout} et \Ie{Soutput}. Elle évalue
  également les expressions R se trouvant dans les commandes
  \cmd{\Sexpr} et elle remplace la commande par le résultat de
  l'expression. Cela produit un fichier \code{.tex}:
\begin{lstlisting}
\section{Commandes R}
...
Seave se révèle particulièrement utile pour créer des graphiques à
partir de R: tout ce que l'on doit conserver dans son fichier
\code{.Rnw}, c'est le code pour créer le graphique (voir l'exercice
!!!!). Il est également possible de contrôler l'exécution des blocs de
code et l'affichage du code source et des résultats.
...
\begin{information}
  ...
  Les personnes intéressées peuvent consulter, par exemple, le %
  \link{http://www.ctan.org/tex-archive/macros/latex/contrib/ulthese}{code source} %
  de la classe \class{ulthese}. La documentation de la classe, son
  code {\LaTeX} ainsi que celui des gabarits, tout se trouve entrelacé
