\documentclass[aspectratio=54,10pt,xcolor=x11names]{beamer}
  ...
  \definecolor{example}{named}{SlateGray1}
  ...
  \setbeamercolor{block title example}{fg=white,bg=Blue3}
  \setbeamercolor{example text}{fg=Blue3,bg=white}
...
\begin{frame}[plain,fragile=singleslide]
  \begin{exercice}
    ...
    \begin{enumerate}
    \item Compiler le document avec la classe \textbf{article}, puis
      avec la classe \textbf{book}. Observer le résultat.
...
\end{lstlisting}
  \item Principales classes:
    \begin{quote}
      ...
      {\color{emphasis} \textbf{memoir}} \\
      {\color{emphasis} \textbf{ulthese}}
    \end{quote}
  \item Principales options:
    \begin{quote}
      ...
      {\color{emphasis} \texttt{article}} (classe \textbf{memoir})
    \end{quote}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Aujourd'hui plus facile d'utiliser d'autres polices, surtout
    ...
    \item voir les fichiers d'exercices et les gabarits de
      \textbf{ulthese} pour des exemples
    \end{itemize}
...
\begin{frame}
  ...
  \item En français, en ligne, libre
    \begin{quote}
      \href{http://fr.wikibooks.org/w/index.php?title=LaTeX&oldid=462659}{%
